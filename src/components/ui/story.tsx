...
>(({ className, ...props }, ref) => {
  ...
  return (
    <button
      ...
      ref={ref}
      className={cn(
        'inline-flex items-center justify-center size-9 rounded-full hover:bg-white/10 transition-colors',
        className,
      )}
      {...props}
    >
      {isPaused ? isEnded ? <ReplyIcon size={18} /> : <PlayIcon size={18} /> : <PauseIcon size={18} />}
    </button>
  );
});
